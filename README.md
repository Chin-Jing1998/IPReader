...
把 88 部知识产权法规与实务文献重构为一座可检索、可批注、可离线阅读的知识库，装成桌面应用。

**默认完全离线**：内容全部内置，无遥测、无账号、无云同步。批注只存在你自己的机器上。唯一可能联网的是「检查更新」——它默认关闭，只在你主动点击、或自行开启「启动时自动检查」后才向 GitHub 查一次版本号，且只取版本号，不下载任何文件、不自动安装。

---

## 内容

全库 **88 部**规范性文件与实务文献，共 **6247 个正文页**；另有 **1035 个术语词条**的关键词索引（24 个主题分组）与一张全库知识图谱（7282 节点、15520 条边）。
...
| 分类 | 部数 | 举例 |
|---|---|---|
| 司法解释与最高人民法院知识产权法庭裁判要旨 | 26 | 侵犯专利权纠纷解释（一）（二）、专利授权确权行政案件规定（一）、知识产权民事诉讼证据规定、裁判要旨摘要 2022–2024 |
| 法律、行政法规、部门规章与规范性文件 | 55 | 商标法、著作权法、反不正当竞争法、反垄断法、植物新品种保护条例、集成电路布图设计保护条例、商标审查审理指南、专利质量评价指南、专利优先审查管理办法 |

条文溯源覆盖 **70 部**法律法规的 **2521 条**条文正文：正文中出现的条号可直接跳转至该条原文，条文侧反向列出引用它的全部章节（3014 条引用关系）。各书正文中的术语自动成链（19104 处行内链接）。

完整书目清单见应用内首页，或经 MCP 的 `list_books` 工具获取。许可状态见 [CONTENT_LICENSE.md](CONTENT_LICENSE.md)。
...
- **术语索引** —— 1035 个词条，按 24 个主题分组（未落类者归「99-综合」），含释义、出处与相关术语
- **翻页与目录** —— 键盘翻页、自动定位的侧栏目录、阅读进度环
- **界面主题** —— 六套古风配色（宣纸 / 水墨 / 青瓷 / 竹林 / 暮山 / 玄夜），各含明暗两态；独立设置页按外观 / 批注 / MCP / 关于四类分栏
- **接入 AI Agent** —— 设置页「MCP」分类按本机真实路径生成接入命令，一键复制即可让 Claude Code、Codex 等工具直接检索本知识库
- **检查更新** —— 设置页「关于」分类内，手动检查或开启启动时自动检查（默认关闭，不开则完全不联网）
...
以下截图取自 macOS 桌面端 1440×900 窗口，界面主题为默认的「宣纸」（第 9 张为其深色态），由 `desktop/shots.cjs` 生成。

![首页](docs/screenshots/01-home.png)

**首页 · 内容总览**　88 部工具书与两个检索入口（图谱总览、关键词索引）在此汇总。左栏是贯穿全库的目录树，右栏是本页关系图与大纲。
...
**术语词条页**　1035 个词条之一。每条给出释义、在各书中的出处（附原文摘录）、相关法条与相关术语；右栏的反向链接列出哪些页面引用了它。
...
**设置 · 界面主题**　六套古风配色——宣纸 / 水墨 / 青瓷 / 竹林 / 暮山 / 玄夜，每套各含浅色与深色两态；明暗模式另有浅色 / 深色 / 跟随系统三档。设置页按外观 / 批注 / MCP / 关于四类分栏。

![深色态章节页](docs/screenshots/09-chapter-dark.png)

**深色态**　与上面第二张为同一页面。页面底色、各栏字体与图谱配色随主题一并切换。

![设置页 · MCP 接入](docs/screenshots/10-settings-mcp.png)

**设置 · 接入 AI Agent**　MCP 分类。两条接入命令（Claude Code 与 Codex CLI）按本机真实路径生成，点右侧按钮即复制；下方列出服务提供的七个工具。图中路径为脱敏占位——即把 dmg 拖进「应用程序」后的标准路径，实机显示的是你自己那台机器上的真实路径。

## 接入 AI Agent（MCP 服务）

除了自己翻阅，这座知识库也可以交给 AI agent 直接调用。仓内的 [`mcp/`](mcp/) 是一个标准 MCP 服务，接上之后，Claude Code、Codex、腾讯 WorkBuddy 等支持 MCP 的工具就能在这 88 部法规与实务文献里检索、读原文、查术语、按条号溯源。
...
与桌面端同样**完全离线**——stdio 是父子进程管道，不开端口、不发请求；`dist/` 是打包好的自包含单文件，无须 `npm install`。冷启动约 290 ms。装了桌面版的话，安装包内已附同一份服务，配置指向应用内路径即可，连仓库都不必克隆。
...
`site/data` 为生成器唯一输入，须与内容修订保持同步。修订源头在外层工作区的各书源 md，传导规程如下：
...
术语链重跑须在带切片源的工作区进行：本仓 `site/` 不含各书的 `_chunks` 切片源，`merge-terms.mjs` 在本仓内直接运行会因 evidence 全部校验失败而静默丢词。重跑后将 `site/data/`、`site/public/content/`、`quartz-kb/content/` 三处产物一并同步回本仓。词条口径另受 `site/data/term-blacklist.json` 的剔除清单约束，改动该清单同样须重跑术语全链。

`site/scripts/lib/topics.mjs` 以**本仓为唯一事实源**：分组层 `TERM_TOPIC_GROUPS` 23 组，加兜底「99-综合」共 24 个主题目录，另有 `manualOnly` 键约定（该键只作人工决策落点，不参与自动归类）。按上述规程「外层重跑解析再搬运」之前，须先确认外层工作区的 `topics.mjs` 与本仓一致，否则已退役的 `examProcedure` 章节主题会原样复活并覆盖分组成果。
...
25 步端到端冒烟。其中**离线护栏**会阻断一切非 `127.0.0.1` 的请求并统计尝试次数——这把"默认离线"从一句承诺变成了可执行的断言，任何改动都不得让它出现非零值。末步专门断言更新检查的默认姿态：区块就位、开关默认未勾选，且冒烟全程不触发它。
...
- 默认不发出任何网络请求。内容、字体、图标全部内置
- 唯一的例外是设置页的「检查更新」：默认关闭，开启或点击后只向 GitHub 取一次最新版本号（不下载文件、不自动安装、不上报任何本机信息）。请求发自主进程，页面侧的 `connect-src 'self'` 保持不变
- 无遥测、无崩溃上报、无使用统计
