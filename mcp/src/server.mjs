...
function registerTools(server) {
  ...
  server.registerTool(
    'browse_toc',
    {
      ...
      // 8 层树，上限 5 时无法一次展开到叶节点。
      inputSchema: z.object({
        root: z.string().optional().describe('书目域键或节点 id；缺省列出全部书目'),
        depth: z.number().int().optional().describe('展开层数，默认 2，上限 8'),
        groupBy: z.enum(['taxonomy']).optional()
          .describe('仅接受 "taxonomy"；缺省 root 时返回体另附按国家/法域/文献类型分层的 grouped 视图，不影响 books 平铺，传了 root 时无效'),
      }),
    },
    async (args) => {
  ...
  server.registerTool(
    'lookup_term',
    {
      title: '查术语',
      description:
        '查询 1035 条专利术语词条：释义、在各书中的出处（含所在章节与原文摘录）、关联法条与相关术语。'
  ...
  server.registerTool(
    'list_books',
    {
      ...
      description: '列出当前开放的书目、各自的节点数与字数，以及被域白名单关闭的书目。先调它可建立全局认知。'
        + '各书目附 country/field/docType 三字段（国家、六标签法域、D1–D6 文献类型；数据包未升级时省略）；'
        + '另附 groups 摘要：按国家→法域→文献类型三级聚合的书目分布统计。',
      inputSchema: z.object({}),
    },
    async () => {
...
function registerResources(server) {
  server.registerResource(
    ...
    new ResourceTemplate('patentkb://node/{id}', {
      // 5488 个节点全量列出会淹没客户端的资源面板，故只登记模板不做枚举，
