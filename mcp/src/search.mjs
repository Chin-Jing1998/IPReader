...
// 其一，术语表精确匹配——查询命中 1035 条术语之一时，把该术语的出处节点提权到结果前列；
...
//
// 阶段 3 改为注册表驱动：69 部有条文规范的法名由数据包 kb.books 的 {domain, short, lawName}
// 在运行时派生（build-data.mjs 已装配该三元组，数据包 schema 零变更），别名池
// 「69 全称 + 65 简称 = 134 项，跨域冲突 0 组」，另加 3 项专利法系遗留简写，运行时共 137 条，
// 按归一后长度降序排列以解 11 对子串包含（「专利法」⊂「专利法实施细则」、「商标法」⊂「商标法实施条例」…）。
//
