...
内容规模：88 部书、7990 节点（正文 6247 + 术语 1743）、正文约 460 万字、图谱边 19318 条、法条正文 2521 条（覆盖 70 部法律法规，另有 3014 条「条文 → 引用章节」反向索引）。
...
> 冷启动实测 354 ms（含解压数据包、建立检索索引与协议握手；节点 7990、书目 88/88，2026-08-30 三跑取中位；术语层由 1035 扩至 1743 条后较 7282 节点形态的 230 ms 增约 54%），仍远低于各客户端的默认启动超时。若客户端的超时设置低于 5 秒，建议按上文示例显式放宽。
