...
async function createWindow() {
  ...
  const win = new BrowserWindow({
    width: 1440,
    height: 900,
    // 最小宽度 1080 → 1260（阶段5.14 施工③，用户按截图指定的「缩小到底」基准）。
    // 判据是图谱总览页图例行的**断行形态**：用户认定 1260 一档的三行断法为可接受下限
    //（首行止于「化学撰写」、次行止于「竞争法·法律」、第三行起于「竞争法·规章」）。
    // 沿宽度以 1px 步进实测，该断行签名成立的连续区间为 [1237, 1279]；取 1260 是
    // 区间内的整十值，距下临界 23px、距上临界 19px，两侧余量均远超 10px 的抖动带，
    // 缩放到底时不会卡在换行临界上来回跳行。
    // 1080 时图例要折成五行、画布被挤到 640px 以下，正是本次要根除的形态。
    // 与显示器的关系：1260 < 1280（最窄的常见 macOS 逻辑宽度），13 吋机型仍可全宽摆放。
    // 副作用（预期内）：视口宽度自此恒 ≥1260 > $tablet 上界 1200px，样式表里的
    // tablet 分支成为不可达路径。刻意不删——它是上游 Quartz 的布局分支，删了会与
    // 上游产生无谓分歧，且浏览器直接打开同一份产物时（非本壳）该分支仍然有效。
    minWidth: 1260,
    minHeight: 700,
