...
// ============================================================
// 用途：把图谱节点配色从「组级 15 色」细化到「书级一书一色」，同时统一微调组代表色
...
// 分配。总书数由 87 增至 88。
//
// 沿革（阶段5.11 波O，2026-08-30）：12 部文献归档下线，总书数 88 → 76，
// ORPHAN_HOST 随 GRAPH_HIDDEN_BOOKS 机制拆除而清空。**本次未运行本脚本回写**——
// 色相按「组内第 k 本书落在 (k+0.5)/n 处」分配，n 变即全组重排，重跑会让 76 部
// 保留书全部换色；两处落点均按下线名单手工摘键，保留书色值逐字不变、两侧仍对齐。
// 下次因配色需求重跑时会按 76 部自然重算，届时全库换色属预期。
//
// ⚠️⚠️ 重跑前必读（阶段5.11 波L 遗留脱节，2026-08-30 登记）⚠️⚠️
// **本文件的组体系仍按旧 15 组书写，与现网组表不符——直接重跑会产出错配的配色。**
//   · 现网 graphSections.ts 的组表已于波L 按「法域 × 文种」由 15 组拆细为 **30 组**，
//     custom.scss 内实有 --graph-section-1..30；而本文件的 BASELINE_SECTION_COLORS
//     每行仅 14 个值、GROUP_TUNE 只覆盖 1..14、SECTION_15_FROM_8 补第 15 组，
//     回写落点②的注释也仍写「--graph-section-1..15」。
//   · 后果：此刻执行 --write 只会写出 1..15，16..30 既不重算也不覆盖，且组号语义
//     已整体位移（旧 15 组的 id 与新 30 组的 id 不是同一套划分），书色与组划分错配。
//   · 因此**重跑前必须先按 graphSections.ts 现网组表重写锚点**：BASELINE_SECTION_COLORS
//     扩到 30 列、GROUP_TUNE 按新组重定、SECTION_15_FROM_8 这类单组派生特例一并复核，
//     并把回写落点②的范围同步改为 1..30。否则勿加 --write。
//   · 只读模式（不带 --write，仅打印可辨性报告）同样按旧组解读，其数字亦不可直接采信。
// ============================================================
...
/**
 * ⚠️ 本表按**旧 15 组**书写（组号 1..14 + 派生的 15），现网组表已拆为 30 组——
 *    重跑前必须先按 graphSections.ts 现网组表重写本表与 BASELINE_SECTION_COLORS，
 *    否则书色与组划分错配。详见文件头「重跑前必读」。以下表项注释描述的是旧组语义。
 *
 ...
 */
// 阶段5.11 波O（2026-08-30）起本表为空：原寄养的 5 部（63/79/87/89/90）已随
// 书目下线从语料与内容目录移除，GRAPH_HIDDEN_BOOKS 机制亦整体拆除，
// 「已摘组但仍需配色」的半态不复存在。表结构保留以备将来再现同类需求。
const ORPHAN_HOST = {}
