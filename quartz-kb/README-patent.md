# 专利知识库 · quartz 定制备忘（patent-kb 分支）

本仓库基于 quartz v4.5.2，承载 7 部专利工具书 + 关键词索引 + 图谱总览共 2217 页的纯离线知识库。
...
# 2. 内容层（../site 内）：重新生成 quartz markdown（2217 页）
node scripts/build-quartz-md.mjs

# 3. 静态资源同步（本目录内）
./sync-content.sh            # ../site/public/content/（2175 个节点详情 JSON）→ quartz/static/content/
