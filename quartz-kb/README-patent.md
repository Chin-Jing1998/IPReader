...
本仓库基于 quartz v4.5.2，承载 76 部知识产权工具书 + 关键词索引 + 图谱总览共 7826 页的纯离线知识库。
...
# 2. 内容层（../site 内）：重新生成 quartz markdown（7826 页）
node scripts/build-quartz-md.mjs

# 3. 静态资源同步（本目录内）
./sync-content.sh            # ../site/public/content/（7706 个节点详情 JSON）→ quartz/static/content/
