        ...
        <section class="kb-settings-pane" data-pane-id="about">
          ...
          */}
          ...
          <section class="kb-settings-sec">
            <h2>图谱总览使用说明</h2>
            <p class="kb-settings-desc">
              「图谱总览」页把全库文献与术语组织为一张知识图谱：图谱节点按所属文献逐书着色，图例按「法域段
              → 文种 →
              组钮」归类、显隐切换仍以组为粒度，节点大小代表层级（书目最大、章节居中、小节与术语最小），节点标签随画面放大逐渐显现。目录树、搜索、关键词索引与知识图谱四者的书目范围一致——图谱中能看到的，正文里都读得到，反之亦然；在库的
              76 部文献全部在图谱中呈现。工具条首行另有「中国 →
            ...
            <div class="kb-settings-guide">
              ...
              <ul>
                ...
                <li>
                  法域标签<strong>可以多选</strong>
                  ：在「全部」态下点任一枚即只看该法域，再点第二枚则两片法域叠加显示，再点已亮的那枚即取消它；取消到一枚不剩、或六枚全部选满时，都会自动回到「全部」态。
                </li>
                <li>
                  每次切换标签，画面只保留已选法域的文献节点、其余暂时隐去，可见子集就地收敛并重新入框，图例行同步收窄为只列这些法域的组。标签行只做法域级筛选，术语层的显示模式不受其影响，仍由术语层三态开关单独控制；筛选之后，下方图例可继续做单组显隐等组级微调。
                </li>
                <li>顶部搜索框输入节点名称，回车或点击「定位」：目标节点描边高亮并平移居中。</li>
                <li>
                  图例共 30 组，按「法域段 → 文种 →
                  组钮」排列：先分专利、商标、著作权、竞争法、品种布图、综合程序六个法域段，段内再按法律、法规、规章、司解、指引五类文种依次列出该格的组钮。主干七部文献各占一组、钮上一律写书名；其余各组，所在格只此一组时钮上直接写两字文种简称，同格有多组时才写各组自己的名称并另立文种小标题（现仅专利法域的「法规」与「指引」两格如此）。术语组列于末位。点击任一组钮可隐藏
                  / 恢复该组全部节点。
                </li>
                <li>
                  图例行首另有「骨架」段控：点击一次隐藏主干七书之外的全部组、只留主干七书骨架，再点一次恢复；单独点选其中某几组后，段控显示为虚线框（部分隐藏）。
                </li>
              ...
              <ul>
                <li>
                  画布左上角标注为「目录」的按钮即抽屉入口。抽屉按「法域 → 书 → 章 →
                  节」三层组织，点其中任一行等同于在图里选中对应节点。
                </li>
                <li>
                  鼠标移上去自动展开、移开自动收起；点抽屉标题栏的「常开」即锁住展开状态，此后移开鼠标也不再收起，该选择会被记住。触屏设备不启用悬停开合，仍按点击展开。
                </li>
                <li>
                  每行左侧的复选框可以多选，<kbd>Cmd/Ctrl + 点击</kbd> 标题同效，最多同时选中 12
                  个节点；被选中的节点及其一跳邻域保持常亮，其余变暗。
                </li>
                <li>
                  标题栏的「清空」一键取消全部多选，点击画布空白处同效；该按钮只在有多选时出现，钮上括号内是当前已选的节点数。选中两个及以上时，右侧面板转为多选摘要，把已选节点列成一排可点的标签，逐枚可移除。
                </li>
              </ul>
              <h3>阅读模式</h3>
              <ul>
                <li>
                  左栏顶部第三枚按钮切换阅读模式。在图谱总览页，左栏整条隐去、画布铺满整个窗口宽度；此时该切换按钮本身随左栏一并隐去，故图谱工具条尾部会另外出现一枚「退出阅读」按钮作为出口。
                </li>
              </ul>
              <h3>页内局部图</h3>
              <ul>
                <li>
                  每个章节页右栏另有局部关联图，仅显示与当前页直接相连的节点，点击即跳转对应页面；节点可直接拖动，单击选中不会扰动已经稳定的布局。
                </li>
                <li>
                  在关联特别密集的页面上，局部图最多显示 120 个关联节点，并在图上给出「已显示 N/M
                  个关联」徽标（例如「已显示 120/154 个关联」）；点徽标即跳到图谱总览查看全貌。
                </li>
              </ul>
            </div>
          </section>
          ...
          <section class="kb-settings-sec">
            ...
            <div class="kb-settings-guide">
              ...
              <ul>
                ...
                <li>
                  <strong>条文行即正文行</strong>
                  ：以单条条文成页的文献（如专利法各条），在目录树里不再带展开箭头，点击直接进入该条正文。
                </li>
                <li>
                  <strong>一键收起分两段</strong>
                  ：第一次点击收起除当前页所在那条路径之外的全部分支，再点一次连当前路径也一并收起。
                </li>
                <li>
                  <strong>展开还原</strong>
                  ：一键收起会一次抹平手工铺开的展开状态，其右侧的还原按钮可把目录树还原到收起之前的样子。
                </li>
                <li>
                  <strong>同级拖拽排序</strong>
                  ：权利类型行、文件归类行与文献行都可以在同级内拖动重排，顺序记在本机。鼠标
                  <strong>悬停</strong>
                  在可排序的行上时，行首才显出拖拽手柄，不悬停即隐去、不占版面。排序只影响目录树里「找书的顺序」，不影响正文「上一篇
                  / 下一篇」的阅读顺序。
                </li>
                <li>
                  <strong>恢复默认排序</strong>
                  ：自定义过顺序后，目录树头部会出现该按钮，点击并确认即回到默认次序，展开状态不受影响。
                </li>
              </ul>
              <h3>全文搜索</h3>
              <ul>
