...
async function createGraphInstance(
  ...
  if (cachedAssembly !== undefined) {
     ...
     *   2) 区分度：实测双向口径下 29.8%（520/1743）的术语跨两域及以上，过滤形同虚设；
     *      仅出边 7.6%（132/1743）跨域，过滤有效。
     *      （20260825 首测为双向 28% / 出边 2.5%，口径与结论不变；上列数值为
     *       20260830 阶段5.11 波O 重建后重测。）
     *   3) 事实自洽：仅出边口径的法域分布如实反映抽取来源。
     *      20260830 实测：专利 891 / 商标 694 / 著作权 102 / 品种布图 98 / 竞争法 69 /
     *      综合程序 64，1743 术语全部有法域归属（词条计数按域重复计，故各域之和 > 1743）。
     *      ⚠️ 20260825 的「术语抽取现仅覆盖专利/商标两域」（当时 852/209/跨域 26/其余四域 0/
     *      1035 词）已被阶段5.9 波4 的四法域提取推翻——六法域现均有词条，编排层的空域提示
     *      因此不再由「整域为 0」触发，只在法域过滤后确无术语时出现。
     *
  ...
  // 键集取自 BOOK_COLORS（76 部文献的目录数字前缀），CSS 变量缺失时逐键回落到
