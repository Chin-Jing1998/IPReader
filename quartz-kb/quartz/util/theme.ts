...
/**
 * 离线定制：生成 CSS 字体变量值。
 * 若字体名中含逗号，说明配置提供的是完整字体栈（如中文系统栈），原样输出；
 * 否则按上游默认行为，将单一字体名加引号并拼接默认回退栈。
 */
function fontStackValue(spec: FontSpecification, fallback: string): string {
...
:root {
  ...
  --titleFont: ${fontStackValue(theme.typography.title || theme.typography.header, DEFAULT_SANS_SERIF)};
  --headerFont: ${fontStackValue(theme.typography.header, DEFAULT_SANS_SERIF)};
  --bodyFont: ${fontStackValue(theme.typography.body, DEFAULT_SANS_SERIF)};
  /* 图谱标签专用字体：跟随标题黑体栈。正文已改宋体，PixiJS 画布标签
     若继续读 --bodyFont 会变宋体（小字号发虚），graph.inline.ts 应改读本变量 */
  --graphFont: ${fontStackValue(theme.typography.header, DEFAULT_SANS_SERIF)};
  --codeFont: ${fontStackValue(theme.typography.code, DEFAULT_MONO)};
}
